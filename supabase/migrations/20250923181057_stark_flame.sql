/*
  ...
  1. New Tables
    - `geofences`
      ...
      - `center` (geography point for center location)
  ...
  3. Indexes
    - Spatial index on center for efficient geographic queries
*/
...
CREATE INDEX IF NOT EXISTS idx_geofences_safe ON geofences(safe);
...
BEGIN
  SELECT center, radius_meters INTO geofence_record
  FROM geofences
...
CREATE OR REPLACE FUNCTION get_nearby_geofences(user_location geography, max_distance_meters integer DEFAULT 5000)
RETURNS TABLE (
  geofence_id uuid,
  geofence_name text,
